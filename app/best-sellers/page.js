...
export default function BestSellers() {
    ...
    const bestSellers = books.filter(book => book.tags && book.tags.includes("bestsellers")).filter((book) => {
    ...
    const sortedBooks = bestSellers.sort((a, b) => {
        switch (filters.sort) {
            case 'bestselling':
        ...
        <div className="flex w-full justify-center items-center flex-col">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-5 xl:grid-cols-5 gap-4">
                {sortedBooks.map((book, index) => (
                    <div
                        key={book.id}
                        className="flex flex-col items-center justify-between bg-[#731212] text-white border rounded-md shadow-md w-[280px] max-w-xs"
                        onMouseEnter={() => setHoveredBookIndex(index)}
                        onMouseLeave={() => setHoveredBookIndex(null)}
                        onClick={() => handleBookClick(book)}
                    >
                        <div className="flex flex-col items-center w-full">
                            <img src={hoveredBookIndex === index ? book.images[1] : book.images[0]} alt={book.title} className="w-full h-72 object-cover mb-4 duration-300" />
            ...
            {selectedBook && (
                <div className="fixed inset-0 bg-black bg-opacity-50 flex justify-center items-center z-50">
                    ...
                    >
                        <button className="absolute top-4 right-4 text-gray-500" onClick={handleCloseModal}>
                            &times;
                        </button>
                        <div className="md:flex w-full justify-center items-center">
                            <div className="flex w-full justify-center items-center">
                                <img src={selectedBook.images[0]} alt={selectedBook.title} className="md:w-1/2 w-full h-auto object-cover rounded-md" />
                            </div>
                            <div className="ml-6 flex flex-col w-full md:px-8">
                                <div>
